...
def mode1():
    ...
    try:
        ...
        if n_size <=0:
            ...
            return
        filter_a = Matrix(n_size)
        while True:
            if filter_a.user_input(name= '필터 A (Cross)'):
                break
            print("다시 시도해주세요. \n")

        filter_b = Matrix(n_size)
        while True:
            if filter_b.user_input(name= '필터 B (Cross)'):
                break
            print("다시 시도해주세요. \n")

        input_pattern = Matrix(n_size)
        while True:
            if input_pattern.user_input(name="검사할 패턴"):
                break
            print("다시 시도해주세요. \n")
...
def run():
    while True:
        print("\n===== Mini NPU Simulator =====")
        print("1. 사용자 직접 입력 모드")
        print("2. data.json 자동 일괄 분석 모드 (준비 중)")
        print("3. 프로그램 종료")

        choice = input("원하는 모드를 선택하세요(1~3) : ").strip()
